...
class SettingsRepository
    ...
    constructor(
        ...
        val enableLogs: Flow<Boolean> =
            context.dataStore.data
                .map { preferences ->
                    preferences[ENABLE_LOGS_KEY] ?: true
                }

        val defaultPort: Flow<String> =
            context.dataStore.data
                .map { preferences ->
                    preferences[DEFAULT_PORT_KEY] ?: Constants.DEFAULT_PORT.toString()
                }

        val logRetentionDays: Flow<String> =
            context.dataStore.data
                .map { preferences ->
                    preferences[LOG_RETENTION_DAYS_KEY] ?: "7"
                }

        val maxLogEntries: Flow<String> =
            context.dataStore.data
                .map { preferences ->
                    preferences[MAX_LOG_ENTRIES_KEY] ?: "1000"
                }

        val autoCleanupEnabled: Flow<Boolean> =
            context.dataStore.data
                .map { preferences ->
                    preferences[AUTO_CLEANUP_ENABLED_KEY] ?: true
                }

        val corsAllowAnyHost: Flow<Boolean> =
            context.dataStore.data
                .map { preferences ->
                    preferences[CORS_ALLOW_ANY_HOST_KEY] ?: false
                }

        val corsAllowedHosts: Flow<String> =
            context.dataStore.data
                .map { preferences ->
                    preferences[CORS_ALLOWED_HOSTS_KEY] ?: ""
                }

        val corsAllowCredentials: Flow<Boolean> =
            context.dataStore.data
                .map { preferences ->
        ...
        private suspend fun getBooleanPreference(key: String): Preferences {
            val prefKey = booleanPreferencesKey(key)
            return context.dataStore.data.first { preferences -> preferences[prefKey] ?: true }
        }
